...
import { usePotentials, InsertPotential } from '@/hooks/usePotentials';
...
export default function PotensiAdmin() {
  const { useFetchPotentials, useCreatePotential, useUpdatePotential, useDeletePotential } = usePotentials();
  const { data: potentials = [], isLoading } = useFetchPotentials();
  ...
  const openCreateModal = () => {
    ...
    reset({ title: '', category: 'Pertanian', description: '' });
    setIsModalOpen(true);
  };
  ...
  const onSubmit = async (data: PotentialFormValues) => {
    try {
      let image_url = photoPreview;

      if (photoFile) {
        image_url = await uploadImage(photoFile, 'potentials');
      }

      const payload: InsertPotential = {
        title: data.title,
    ...
    <div className="bg-slate-50 dark:bg-slate-900 min-h-screen p-6">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700">
          <div>
            <h1 className="text-2xl font-bold text-slate-800 dark:text-white">Potensi Desa</h1>
            <p className="text-slate-500 text-sm mt-1">Kelola data potensi desa.</p>
          </div>
          <button 
          ...
          >
            <Plus className="w-5 h-5" /> Tambah Potensi
          </button>
        </div>
          ...
          {isLoading ? (
            ...
            potentials.map((item) => (
              ...
              >
                ...
                <span className="inline-block px-3 py-1 bg-sky-100 text-sky-700 dark:bg-sky-900/30 dark:text-sky-400 rounded-full text-xs font-bold w-max mb-2">
                  {item.category}
                </span>
                <h3 className="text-lg font-bold text-slate-800 dark:text-white">{item.title}</h3>
                {item.description && <p className="text-sm text-slate-500 mt-2 line-clamp-3">{item.description}</p>}
              </motion.div>
            ))
          ) : (
            ...
            >
              <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/80">
                <h2 className="text-xl font-bold text-slate-800 dark:text-white">
                  {selectedId ? 'Edit Potensi' : 'Tambah Potensi'}
                </h2>
              </div>
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                ...
                <div>
                  <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Judul Potensi</label>
                ...
                <div>
                  ...
                  <select {...register('category')} className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-emerald-500 text-slate-700 dark:text-slate-200">
                ...
                <div>
                  ...
                  <textarea {...register('description')} rows={4} className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-emerald-500 text-slate-700 dark:text-slate-200 resize-none" />
                </div>
