...
export function VillageStats({ variant = 'default' }: { variant?: 'default' | 'card' }) {
  ...
  const stats = [
    ...
    { value: profile?.rt ? profile.rt.toString() : '45', label: 'RT' },
  ];
  ...
  return (
    <section className={`relative overflow-hidden ${isCard ? 'py-12 bg-emerald-500/10 dark:bg-emerald-500/10 rounded-3xl border border-emerald-500/20 backdrop-blur-sm mt-4' : 'py-24 bg-primary-950 dark:bg-black'}`}>
      {!isCard && <div className="absolute inset-0 bg-gradient-to-br from-white/5 to-transparent opacity-50" />}
      <div className="container mx-auto px-4 relative z-10">
        <div className={`grid grid-cols-2 md:grid-cols-5 gap-8 ${isCard ? 'divide-x divide-emerald-500/20' : 'divide-x divide-white/10'}`}>
          {stats.map((s, i) => (
            <motion.div 
              key={i}
